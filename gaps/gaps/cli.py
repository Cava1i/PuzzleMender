...
def run(
    ...
    if input_puzzle is None:
        click.echo(f"Failed to read image with imdecode: {puzzle}")
        # 尝试备用方法读取
    ...
    cv.imencode('.png', output_image)[1].tofile(solution)
...
def create(image: str, puzzle: str, size: int) -> None:
    ...
    input_image = cv.imdecode(np.fromfile(image, dtype=np.uint8), cv.IMREAD_COLOR)
    ...
    cv.imencode('.png', output_image)[1].tofile(puzzle)
